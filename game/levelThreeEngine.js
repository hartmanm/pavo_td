...
//950, 950
var game = new Phaser.Game(884, 884, Phaser.AUTO, 'lvl3', { preload: preload, create: create, update: update, render: render });
...
game.load.spritesheet('boom', 'game/three/explosion_transparent.png', 64, 64, 23);
game.load.spritesheet('bomb_explode', 'game/three/explosion_transparent.png', 64, 64, 23);
...
buyBombSprite = game.add.sprite(16, game.height - 346, 'buyBomb');
...
buyBombText = game.add.text(16, game.height - 378, '', {font: '16px Arial', align: 'center'});
buyBombText.text = 'Bomber: $55';

buyTurretSprite = game.add.sprite(16, game.height - 474, 'buyTurret');
buyTurretSprite.inputEnabled = true;
buyTurretSprite.alpha = 0.2;
buyTurretSprite.events.onInputDown.add(addTurret, this);
buyTurretText = game.add.text(16, game.height - 506, '', {font: '16px Arial', align: 'center'});
buyTurretText.text = 'Turret: $20';

buyIceSprite = game.add.sprite(16, game.height - 602, 'buyIce');
buyIceSprite.inputEnabled = true;
buyIceSprite.alpha = 0.2;
buyIceSprite.events.onInputDown.add(addIce, this);
buyIceText = game.add.text(16, game.height - 634, '', {font: '16px Arial', align: 'center'});
...
logo = game.add.sprite(226, 400, 'logo');
...
if (newBomber) {
newBomber.reset(buyBombSprite.x + 116, buyBombSprite.y + 32)
bomberBuildOk = false;
...
if (newTurret) {
newTurret.reset(buyTurretSprite.x + 116, buyTurretSprite.y + 32);
turretBuildOk = false;
...
if (newIce) {
newIce.reset(buyIceSprite.x + 116, buyIceSprite.y + 32);
iceBuildOk = false;
newIce.bringToTop();
/*if (game.input.activePointer.isDown) {
...
if( (aliveCreeps == 0) && (currentWave == totalWave) )
{
game.debug.text('LEVEL COMPvarE! ', 32, 32);
game.debug.text('Lives: ' + lives, 32, 145, 'magenta');
if (!gameOver) {
gameOver = true;
}
